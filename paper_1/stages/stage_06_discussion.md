...
The completed evidence shows five separate findings. First, the V11 posterior contains nonlinear distance responses, with the strength and sign certainty of curvature varying by response and fold. Second, algebraic stationary points can be calculated, but their uncertainty differs substantially and can be large near weak quadratic curvature. Third, empirical support and response-curve shape provide information beyond the existence of a stationary point. Fourth, fold-level OOF performance varies across the along-belt partitions even though pooled V11 performance is competitive. Fifth, the same frozen OOF predictions show different discrimination patterns in NRB_3a and NRB_3b.

This study contributes an integrated treatment of these quantities rather than treating them as interchangeable evidence.
...
## 6.2 Implications for Daly's fault-distance hypothesis
...
## 6.3 $D^*$ and empirical support

The algebraic stationary point is useful because it makes the location implied by the fitted quadratic explicit. Its interpretation is strongest when curvature is directionally supported, the posterior distribution is concentrated, the response shape agrees with the stationary-point classification and the point lies within observed support. The NRB_3a lithology response approaches this combination most closely. The fault-distance cases do not provide the same uniform evidence, particularly where curvature is sign-uncertain or the $D^*$ interval is broad.

Empirical support does not convert $D^*$ into a causal geological threshold. It only establishes that the fitted stationary point falls within a range represented by the relevant training observations with a stated posterior probability.

## 6.4 Spatial non-stationarity and geological domains

The variation in posterior coefficients, curvature probabilities and response shapes provides evidence that the fitted relationships are not identical across spatially separated units. The hierarchical structure is appropriate for representing that variation with partial pooling, but it cannot identify geological causation by itself. The Daly domains add geological stratification to the interpretation, while the zero-positive domains limit the discrimination evidence that can be obtained from the current modeling population.
...
## 6.6 Relationship between predictive evidence and geological interpretation

The domain-stratified AUC answers how well the already-generated spatially held-out predictions discriminate deposits from non-deposits within each represented Daly domain. It does not identify a best domain, establish calibration, infer a causal mechanism or determine an exploration distance. The NRB_3a and NRB_3b contrast is evidence of heterogeneous predictive behavior within the evaluated OOF population, not a universal ranking of models or geological settings.

## 6.7 Data composition and domain limitations
...
The domain-stratified results are strongest where positive and negative observations coexist. NRB_3a contains 107 of the 138 positive observations and NRB_3b contains 31; CRZ, NKB, SRB and MMSB contain none. These domain counts describe representation within the modeling population and should not be generalized automatically to the full Copperbelt geological system.

The four-fold along-belt validation samples geographic transfer through the selected partitions, not every possible future setting. The observational design limits causal interpretation. The remaining statistical limitations are uncertainty in $D^*$, weak or sign-uncertain quadratic curvature in several cases, and the distinction between predictive discrimination and geological explanation.
...
The results support a reporting practice in which nonlinear curvature, algebraic stationary points, empirical support, response shape, spatial variation and held-out discrimination are presented separately. This avoids turning a fitted curve feature into a universal exploration rule and avoids using pooled predictive performance to conceal regional differences. Geological-domain stratification is informative when interpreted as a grouping of existing spatial OOF predictions and when its data-support limits are made explicit.
